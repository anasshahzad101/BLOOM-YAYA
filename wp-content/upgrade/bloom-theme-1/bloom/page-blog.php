...
<div class="whisper"><span class="whisper-track"><span>GCC-wide delivery</span><span class="dot">âœ¦</span><span>Kingdom Mall, Riyadh</span><span class="dot">âœ¦</span><span>Complimentary styling consultation</span><span class="dot">âœ¦</span><span>Spring / Summer 2026</span></span></div>
...
<section class="ph-hero" id="phHeroSection" aria-labelledby="phTitle">
  <div class="ph-hero-img" style="background-image:url('https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=2000&q=85&auto=format');" role="img" aria-label="The Bloom Journal"></div>
  ...
  <div class="ph-hero-ct">
    ...
    <p class="ph-sub" id="phSub">Stylist notes, designer profiles, and seasonal guides â€” written with the same restraint we bring to the showroom.</p>
  </div>
</section>
...
<?php if ( $featured->have_posts() ) : $featured->the_post();
  ...
  if ( ! $f_thumb ) $f_thumb = get_post_meta( get_the_ID(), '_bloom_post_image', true );
  if ( ! $f_thumb ) $f_thumb = 'https://images.unsplash.com/photo-1581044777550-4cfa60707c03?w=1600&q=85&auto=format';
...
<section class="bl-feat">
  <a href="<?php the_permalink(); ?>" class="bl-feat-card rv">
    ...
    <div>
      <div class="bl-feat-meta"><?php echo esc_html( $f_cat ); ?><span class="sep">Â·</span><?php echo esc_html( get_the_date( 'F Y' ) ); ?><span class="sep">Â·</span><?php echo (int) max(1,round( str_word_count(wp_strip_all_tags(get_the_content()))/220 )); ?> min read</div>
      <h2 class="bl-feat-h"><?php the_title(); ?></h2>
      <p class="bl-feat-p"><?php echo esc_html( wp_trim_words( get_the_excerpt(), 38, 'â€¦' ) ); ?></p>
      <span class="bl-feat-cta">Read essay â†’</span>
    </div>
  </a>
</section>
  ...
  <div class="bl-grid" id="blGrid">
    ...
    ?>
      <a class="bl-card rv" href="<?php the_permalink(); ?>">
        ...
        <div class="bl-card-meta"><?php echo esc_html( get_the_date( 'F Y' ) ); ?><span class="sep">Â·</span><?php echo (int) max(1,round( str_word_count(wp_strip_all_tags(get_the_content()))/220 )); ?> min read</div>
        <h3 class="bl-card-h"><?php the_title(); ?></h3>
        <p class="bl-card-p"><?php echo esc_html( wp_trim_words( get_the_excerpt(), 28, 'â€¦' ) ); ?></p>
        <span class="bl-card-cta">Read essay â†’</span>
      </a>
    <?php endwhile; wp_reset_postdata(); else : ?>
      <div style="grid-column:1/-1;text-align:center;padding:60px 20px;color:var(--txt-gray);">
        ...
        <p style="font-size:15px;max-width:480px;margin:0 auto 20px;">Publish your first blog post from <strong>Posts â†’ Add New</strong> in the WordPress admin.</p>
        <a href="<?php echo esc_url( admin_url('post-new.php') ); ?>" class="btn-gold"><span>Write a post</span><span class="arrow">â†’</span></a>
      </div>
    <?php endif; ?>
  </div>
</section>
...
<section class="news">
  <div class="news-in rv">
    ...
    <p class="news-p">New essays, designer profiles, and seasonal stylist notes â€” straight to your inbox. Twice a month, never more.</p>
...
<footer class="footer">
  <div class="ft-top">
    ...
    <div><div class="ft-col-title">Collection</div><a href="<?php echo esc_url( home_url("/shop/") ); ?>" class="ft-link">Shop â€” All Pieces</a>
  ...
  <div class="ft-bot"><span>Â© 2026 BLOOM Fashion Boutique Â· CR: XXXXXXXXXX Â· VAT: XXXXXXXXXXXXXXX</span><div class="ft-social"><a href="#">Instagram</a><a href="#">Snapchat</a><a href="#">TikTok</a></div><span>bloom-fashions.com</span></div>
</footer>

<script>
/* Articles data */
const ARTICLES=[
  {cat:'designer',catLabel:'Designer Story',title:"Inside Zuhair Murad's Beirut atelier â€” a quiet revolution in tulle.",excerpt:'A private visit to the Spring 2026 collection, and a conversation about the discipline of restraint in modern couture.',date:'February 2026',read:'8 min read',img:'https://images.unsplash.com/photo-1581338834647-b0fb40704e21?w=900&q=85&auto=format'},
  {cat:'style',catLabel:'Style Guide',title:'Beyond black tie â€” the new vocabulary of the wedding guest.',excerpt:'How to dress for the three-day celebrations that define a Saudi spring â€” from the welcome dinner to the henna to the wedding itself.',date:'January 2026',read:'4 min read',img:'https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=900&q=85&auto=format'},
  {cat:'designer',catLabel:'Designer Story',title:"Elie Saab Spring 2026 â€” what the runway didn't show you.",excerpt:'A reflection on the unseen seven pieces from Saab\'s Paris show that will live, quietly, on our racks first.',date:'February 2026',read:'6 min read',img:'https://images.unsplash.com/photo-1581044777550-4cfa60707c03?w=900&q=85&auto=format'},
  {cat:'editorial',catLabel:'Editorial',title:'The five designers every Riyadh wardrobe needs.',excerpt:'Our stylist team on the houses that have earned a permanent place in the modern Kingdom wardrobe â€” and why.',date:'December 2025',read:'7 min read',img:'https://images.unsplash.com/photo-1485968579580-b6d095142e6e?w=900&q=85&auto=format'},
  {cat:'atelier',catLabel:'Atelier Note',title:'On alterations â€” why we never let a gown leave imperfect.',excerpt:'A behind-the-scenes look at the in-house seamstress team, and the seventy-two hours that turn a gown into your gown.',date:'January 2026',read:'3 min read',img:'https://images.unsplash.com/photo-1494578379344-d6c710782a3d?w=900&q=85&auto=format'},
  {cat:'style',catLabel:'Style Guide',title:'Dressing for the desert evening â€” fabrics, layers, and the case for silk.',excerpt:'A practical guide to the spring temperature swings that catch every wedding guest off guard.',date:'November 2025',read:'5 min read',img:'https://images.unsplash.com/photo-1572804013427-4d7ca7268217?w=900&q=85&auto=format'},
  {cat:'editorial',catLabel:'Editorial',title:'The quiet revolution in modern tulle.',excerpt:'Why the houses we love are returning to the most fragile fabric in fashion â€” and what that says about the next five years of couture.',date:'October 2025',read:'9 min read',img:'https://images.unsplash.com/photo-1539109136881-3be0616acf4b?w=900&q=85&auto=format'},
  {cat:'designer',catLabel:'Designer Story',title:'Reem Acra on the bridal gown as a private love letter.',excerpt:'A conversation with the New York designer on why the most personal gowns are the ones that almost nobody sees.',date:'September 2025',read:'6 min read',img:'https://images.unsplash.com/photo-1566174053879-31528523f8ae?w=900&q=85&auto=format'},
  {cat:'atelier',catLabel:'Atelier Note',title:'A Saturday at BLOOM â€” the rhythm of the showroom.',excerpt:'From the first coffee at 9:45 to the last fitting at 9:30 PM â€” what an unrushed day inside Kingdom Mall, Floor 2 actually looks like.',date:'November 2025',read:'4 min read',img:'https://images.unsplash.com/photo-1539109136881-3be0616acf4b?w=900&q=85&auto=format'},
];
...
function renderArticles(filter='all'){
  ...
  blGrid.innerHTML=items.map((a,i)=>`
    <article class="bl-card rv rv-d${(i%3)+1}" data-cat="${a.cat}">
      ...
      <div class="bl-card-meta">${a.date}<span class="sep">Â·</span>${a.read}</div>
...
/* Preloader â€” fade regardless of slow images */
...
/* â•â•â• TITLE LETTER-MASK REVEAL â•â•â• */
setTimeout(()=>{
...
/* â•â•â• SUBTITLE PER-WORD REVEAL â•â•â• */
...
/* â•â•â• 3D TILT ON ARTICLE CARDS â•â•â• */
...
/* â•â•â• IMAGE CLIP-PATH ENTRANCE â•â•â• */
...
/* â•â•â• FEATURED IMAGE PARALLAX â•â•â• */
...
/* â•â•â• CURSOR "READ" STATE OVER ARTICLE CARDS â•â•â• */
...
/* â•â•â• ENHANCE AFTER RENDER â•â•â• */
...
/* Server now renders the article grid â€” just bind reveal observer */
